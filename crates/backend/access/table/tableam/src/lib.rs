...
/// `SELECT ... FOR UPDATE` and its siblings on an objkv table. A lock that no
/// other session can see protects nothing, so the statement is refused rather
/// than allowed to succeed as a no-op.
fn objkv_row_lock_unsupported(mode: LockTupleMode) -> Box<::types_error::PgError> {
    let clause = match mode {
        LockTupleMode::LockTupleKeyShare => "FOR KEY SHARE",
        LockTupleMode::LockTupleShare => "FOR SHARE",
        LockTupleMode::LockTupleNoKeyExclusive => "FOR NO KEY UPDATE",
        LockTupleMode::LockTupleExclusive => "FOR UPDATE",
    };
    Box::new(
        ::types_error::PgError::error(
            "SELECT ... FOR UPDATE/SHARE is not supported on objkv tables".to_string(),
        )
        .with_detail(format!(
            "The statement asked for {clause} on a row of an objkv table. objkv has no row \
             locks: a lock taken here would not be seen by another session, so it would \
             protect nothing."
        ))
        .with_hint(
            "UPDATE the row instead, which registers a conflict when the transaction \
             commits, or keep the table in the heap access method. A foreign key that \
             references an objkv table runs FOR KEY SHARE and is refused the same way."
                .to_string(),
        )
        .with_sqlstate(::types_error::ERRCODE_FEATURE_NOT_SUPPORTED),
    )
...
pub fn table_tuple_lock<'mcx>(
    ...
    match am(rel) {
        ...
        TableAm::Objkv => {
            // A row-lock statement -- SELECT ... FOR UPDATE / NO KEY UPDATE /
            // SHARE / KEY SHARE, including the FOR KEY SHARE a foreign-key
            // check runs against the referenced table -- arrives from LockRows
            // with this flag. objkv has no row locks: nothing taken here would
            // be seen by another session, so two lockers would both proceed
            // and neither would ever conflict. That is refused, not faked.
            if flags & TUPLE_LOCK_FLAG_LOCK_UPDATE_IN_PROGRESS != 0 {
                return Err(objkv_row_lock_unsupported(mode));
            }
            // Every other caller (ON CONFLICT DO UPDATE, a BEFORE ROW trigger,
            // the recheck after a concurrent update) locks a row it is about
            // to write in this same statement. That write is what objkv
            // validates when the commit object lands, and a concurrent writer
            // makes the loser retry. So the lock reads the row and reports
            // honestly if it is gone.
            let _ = (cid, wait_policy);
            *tmfd = TM_FailureData::default();
...
pub fn table_relation_size(rel: &Relation<'_>, forkNumber: ForkNumber) -> PgResult<u64> {
    match am(rel) {
        ...
        // from it told the planner every such relation was empty until its
        // first ANALYZE. A zero-row estimate is a wrong plan with no error to
        // notice it by, so report the live bytes actually stored in the
        // bucket.
...
pub fn table_scan_analyze_next_block<'mcx>(
    ...
    match scan {
        ...
        // ANALYZE on objkv samples through getnextslot
        // (`analyze::objkv_acquire_sample_rows`), never the block sampler.
        TableScanDesc::Objkv(_) => panic!("objkv: heap block-sampler path; ANALYZE samples through getnextslot"),
...
pub fn table_relation_estimate_size(
    ...
    // live in the bucket, so answer from there (ANALYZE refreshes reltuples,
    // but the density math would still see an empty file). `pages` is
    // notional: it exists because the cost model is written in 8KB pages, not
    // because anything is paged. allvisfrac 0.0 -- no visibility map, so
    // index-only-scan costing assumes every row is fetched from the table.
